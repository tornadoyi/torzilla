...
def init_rpc(
    ...
    assert_type(world_size, int)
    assert_type(rank, int)
    assert_type(backend, str, null=True)
    assert_type(init_method, str, null=True)
    assert_type(rpc_timeout, int, float, null=True)
    assert_type(num_worker_threads, int, null=True)
    assert_type(name, str, null=True)
    ...
    if backend is None or backend == _rpc.BackendType.TENSORPIPE:
        ...
        opt_args = pick_args(args, args.keys(), drop_none=True)
